...
class FlaxHyperRobertaEmbeddings(FlaxRobertaEmbeddings):
    ...
    def setup(self):
        ...
        self.target_priors_projection = nn.Dense(self.config.hidden_size, dtype=self.dtype)
    ...
    def __call__(self,
        ...
        if self.config.embed_target_priors:
            ...
            target_priors_embeddings = self.target_priors_projection(target_priors[..., None] / self.prior_normalization_constant)
            hidden_states = jnp.concatenate([hidden_states, target_priors_embeddings[:, None, :]], axis=1)
        ...
        if self.config.embed_lang_id:
            ...
            hidden_states = jnp.concatenate([hidden_states, lang_embeddings[None, None, :].repeat(hidden_states.shape[0], axis=0)], axis=1)
            attention_mask = jnp.concatenate([
                attention_mask, 
                jnp.ones((attention_mask.shape[0], 1), dtype=attention_mask.dtype)
            ], axis=1)
...
class FlaxHyperRobertaAttention(FlaxRobertaAttention):
    ...
    def setup(self):
        self.self = FlaxHyperRobertaSelfAttention(self.config, causal=self.causal, dtype=self.dtype)
        if self.with_output:
...
class FlaxHyperRobertaOutput(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.language_adapter_bottleneck_dim > 0:
            self.lang_adapter_down_weights = self.param(
                ...
                (self.config.n_langs, self.config.hidden_size, self.config.language_adapter_bottleneck_dim),
                self.dtype,
            )
            ...
            self.lang_adapter_up_weights = self.param(
                ...
                jax.nn.initializers.lecun_normal(),
                (self.config.n_langs, self.config.language_adapter_bottleneck_dim, self.config.hidden_size),
                self.dtype,
            )
    ...
    def __call__(self, hidden_states, attention_output, lang_index, deterministic: bool = True):
        ...
        if self.config.language_adapter_bottleneck_dim > 0:
            ...
            la_down_weights = jnp.take(self.lang_adapter_down_weights, lang_index, axis=0)
            ...
            la_down_output = jnp.einsum("...i,io->...o", hidden_states, la_down_weights) + la_down_bias
            la_down_output = self.activation(la_down_output)

            la_up_output = jnp.einsum("...i,io->...o", la_down_output, la_up_weights) + la_up_bias
...
class FlaxHyperRobertaLayer(FlaxRobertaLayer):
    ...
    def setup(self):
        self.attention = FlaxHyperRobertaAttention(self.config, causal=self.config.is_decoder, dtype=self.dtype)
        ...
        if self.config.add_cross_attention:
            self.crossattention = FlaxHyperRobertaAttention(self.config, causal=False, dtype=self.dtype)
        if self.config.add_inter_token_attention:
            self.inter_token_attention_down = FlaxHyperRobertaAttention(self.config, causal=False, dtype=self.dtype)
            self.inter_token_attention_up = FlaxHyperRobertaAttention(self.config, causal=False, dtype=self.dtype)
    ...
    def __call__(
        ...
        if self.config.add_inter_token_attention:
            if self.config.inter_token_attention_bias_by_priors:
                attention_bias = target_priors[None] * self.config.inter_token_attention_bias_scaler
            else:
                attention_bias = None
            ...
            blocks = self.inter_token_attention_down(
                ...
                attention_bias=attention_bias,
                key_value_states=attention_output[None, :, 0], # (1 x n_tokens x hidden_size)
                deterministic=deterministic,
            )[0][0]
        ...
        hidden_states = self.output(hidden_states, attention_output, lang_index, deterministic=deterministic)
...
class FlaxHyperRobertaLayerCollection(FlaxRobertaLayerCollection):
    ...
    def setup(self):
        ...
        if self.gradient_checkpointing:
            FlaxRobertaCheckpointLayer = remat(FlaxHyperRobertaLayer, static_argnums=(8, 9, 10))
            self.layers = [
    ...
    def __call__(
        ...
        inter_token_blocks = self.inter_token_blocks if self.config.add_inter_token_attention else None
