#!/bin/bash
# Pair 4 (ALM-only): Mistral-7B -> TinyLlama-1.1B (LoRA, 15 epochs, batch 8).
...
NAME=mistral-7b_to_tinyllama_alm

python3 pytorch_cross_tokenizer_distill.py \
    --config=configs/mistral-7b_to_tinyllama_alm_distill.yaml \
    --overrides \
