#!/bin/bash
# Pair 1 (ALM-only): Qwen1.5-1.8B -> GPT2-medium (full fine-tune, 20 epochs, batch 8).
...
NAME=qwen1.5-1.8b_to_gpt2-medium_alm

python3 pytorch_cross_tokenizer_distill.py \
    --config=configs/qwen1.5-1.8b_to_gpt2-medium_alm_distill.yaml \
    --overrides \
