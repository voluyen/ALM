#!/bin/bash
# Pair 0 ablation (MTA all-word): Qwen1.5-1.8B -> GPT2 (124M).
# All 3 projectors word-level, span-level disabled.
set -e
...
python3 pytorch_cross_tokenizer_distill.py \
    --config=configs/qwen1.5-1.8b_to_gpt2_mta_all_word_distill.yaml \
    --overrides \
