#!/bin/bash
# Pair 0 ablation (MTA no-weight): Qwen1.5-1.8B -> GPT2 (124M).
# Spans mean-pooled uniformly (wo_span_weight=true); layer split unchanged.
set -e

GPUS=(0)
export CUDA_VISIBLE_DEVICES=$(IFS=,; echo "${GPUS[*]}")

NAME=qwen1.5-1.8b_to_gpt2_mta_no_weight

python3 pytorch_cross_tokenizer_distill.py \
    --config=configs/qwen1.5-1.8b_to_gpt2_mta_no_weight_distill.yaml \
    --overrides \
