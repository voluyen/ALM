...
from tokenkit.compat.hyper_roberta import HyperRobertaConfig, FlaxHyperRobertaModule
...
class Rescaler(nn.Module):
    ...
    @staticmethod
    def scale_to(x, target=None, target_stds=None, target_means=None, epsilon=1e-8):
...
class Hypernet(nn.Module):
    ...
    def init_rescaler(
        ...
        _, _, num_embeddings, n_embd = embeddings.shape
        ...
        win, bin = self.in_scaler.scale_to(
            embeddings.reshape(-1, num_embeddings * n_embd),
            target_stds=jnp.full(
                num_embeddings * n_embd, fill_value=self.embed_init_range
            ),
            target_means=jnp.zeros(num_embeddings * n_embd),
        )
        ...
        pred = self.apply(
            params,
            embeddings,
            jnp.ones(embeddings.shape[:2], dtype=bool),
        )
        pred_in = pred[:, 0]
    ...
    def __call__(self, embeddings, attention_mask, vocab_indices=None):
        if not self.config.hn_use_attention_mask:
            attention_mask = jnp.ones_like(attention_mask)

        vocab_size, seq_length, num_embeddings, n_embd = embeddings.shape

        if not self.config.hn_embed_using_source_embeddings:
            raise NotImplementedError()

        embeddings = embeddings.reshape(vocab_size, seq_length, num_embeddings * n_embd)

        if self.config.hn_rescale_embeddings:
            source_embeds = self.in_scaler(embeddings)
        else:
            source_embeds = embeddings

        hidden_states = self.model(
            input_ids=jnp.zeros_like(attention_mask, dtype=jnp.int32),
            inputs_embeds=self.input_projection(source_embeds),
        ...
        if self.config.hn_concat_last_hidden_state:
            hidden_states = hidden_states.reshape(attention_mask.shape[0], -1)
        else:
            hidden_states = hidden_states[:, 0]
        ...
        if getattr(self.config, "hn_predict_bias", False):
            ...
            predicted_bias = jnp.zeros_like(attention_mask[..., 0], dtype=self.dtype)

        predicted_embeddings = predicted_embeddings_in[:, None, :]
